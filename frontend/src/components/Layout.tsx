...
import { Menu, X, Train, LayoutDashboard, Users, UserCircle, LogOut } from 'lucide-react';
            ...
            <nav className="hidden md:flex items-center gap-1">
              {navLinks.map(({ to, label, icon: Icon }) => (
                <Link
                  ...
                  activeProps={{ className: 'flex items-center gap-1.5 px-3 py-2 rounded-md text-sm font-medium text-white bg-white/15 border border-white/20' }}
                >
            ...
            <div className="px-4 py-3 space-y-1">
              {navLinks.map(({ to, label, icon: Icon }) => (
                <Link
                  ...
                  activeProps={{ className: 'flex items-center gap-2 px-3 py-2.5 rounded-md text-sm font-medium text-white bg-white/15' }}
                >
                  <Icon className="w-4 h-4" />
                  {label}
                </Link>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
            ...
            <div className="text-center sm:text-right">
              <p>This platform only facilitates connections. Official transfers are governed by Indian Railways rules.</p>
              <p className="mt-1">
