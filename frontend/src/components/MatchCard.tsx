...
import {
  User, MapPin, Building2, Train, Phone, Mail,
  EyeOff, ChevronDown, ChevronUp, ArrowRight
} from 'lucide-react';
      ...
      <CardContent className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {profile.designation && (
                <p className="text-muted-foreground text-xs mt-0.5">{profile.designation}</p>
              )}
            </div>
          </div>
          <Badge variant="secondary" className="text-xs bg-primary-50 text-primary-700 border-primary-100 flex-shrink-0">
            Mutual Match
          </Badge>
        </div>
          ...
          {profile.railwayZone && (
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">{profile.railwayZone}</p>
              </div>
            </div>
          )}
          {profile.division && (
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">{profile.division}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-secondary rounded-xl p-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1">
              <p className="text-xs text-muted-foreground mb-0.5">Wants to go to</p>
              <div className="flex items-center gap-1.5">
          ...
          <div className="mt-3 p-3 bg-primary-50 rounded-xl border border-primary-100 animate-fade-in">
            {profile.contactVisible ? (
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <p className="text-sm">This employee has chosen to keep their contact details private.</p>
              </div>
            )}
          </div>
        )}
