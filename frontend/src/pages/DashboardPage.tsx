...
import { useGetCallerUserProfile, useGetDashboardStats } from "../hooks/useQueries";
import {
  Users,
  UserCheck,
  ArrowRight,
...
export default function DashboardPage() {
  ...
  if (!isLoading && (profileError || statsError)) {
    ...
    const isUnauthorized = errMsg.includes("Unauthorized") || errMsg.includes("not found");
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-card">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            {[
              ...
              { label: "Current Posting", value: profile.currentPostingLocation },
              { label: "Desired Posting", value: profile.desiredPostingLocation },
            ].map(({ label, value }) => (
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              <span className="font-medium text-foreground">{currentPosting}</span>
              {" → "}
